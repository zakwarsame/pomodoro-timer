...
function App() {
  ...
  const gradients = {
    pomodoro: 'linear-gradient(180deg, rgba(128, 91, 143, 0.6) 0%, rgba(53, 92, 125, 0.8) 100%)',
    shortBreak: 'linear-gradient(180deg, rgba(91, 143, 128, 0.6) 0%, rgba(53, 125, 104, 0.8) 100%)',
    longBreak: 'linear-gradient(180deg, rgba(91, 110, 143, 0.6) 0%, rgba(53, 74, 125, 0.8) 100%)'
  };
  ...
  const handleFileChange = (event) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      document.documentElement.style.setProperty('--background-image', `url('${fileUrl}')`);
  ...
  const switchMode = (mode) => {
    ...
    if (!customWallpaper) {
      document.documentElement.style.setProperty('--background-image', `url('${wallpapers[mode]}')`);
    }
    
    // Always update the gradient
    document.documentElement.style.setProperty('--overlay-gradient', gradients[mode]);
  };
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--background-image', `url('${wallpapers[timerMode]}')`);
    document.documentElement.style.setProperty('--overlay-gradient', gradients[timerMode]);
